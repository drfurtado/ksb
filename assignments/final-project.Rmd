...
1. [Link](final-project-template.docx) to the template you `must` use to complete your project - no exception.
...
2. Scores were also fabricated using the test’s population mean and standard deviation for the Wonderlic^[Wonderlic test - Wikipedia." https://en.wikipedia.org/wiki/Wonderlic_test. Accessed 21 Oct. 2020.] variable. Due to privacy, the data for this variable are not publicly available. 
...
##### 1. Identify the dependent and independent variable(s)
...
Next, you must decide whether you will be testing your hypothesis using the one-tailed or the two-tailed test. Once this is decided, you are ready to state the Ho and the Ha:
...
##### 3. Choose the appropriate statistical model to test the hypothesis(ses)
...
##### 4. Run the chosen model using JASP

Use JASP to open the data set and run the statistical model selected in `#3` above.  

Note that the relevant graphs/tables generated by JASP should be used to support your `write-up`. 

----

##### 5. Data Analysis and Interpretation

Under this section, you will be asked to interpret the results based on the output generated by JASP.

**This is the most important section of your report. Ensure to be throughout when writing this section.**

> Hint: Please download and read [this article](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4062332/pdf/biochem-med-22-1-15-3.pdf) that covers the best practices of Methods/Results/Conclusion write-ups.

Include the following and in the sequence presented below:

1. Statistical Analysis

    - Refer to the NCBI article (link above) to learn what to include here. 

2. Results^[Ensure to phrase your results following the APA Style → https://bit.ly/2HirVLv] 

a. You will be required to add `at least two` tables and `one graph` to enhance the `Results` section. Tables and graphs must come from JASP and be related to the statistical model used to test your hypothesis(es).  
...
##### 6. References (following the 6th APA Style)

All sources consulted must be properly cite (in-text citation following APA Style) and the sources must be listed under `References`.

----

#### Appendices

**A. Correlation reference criteria**

When evaluating the size of a bivariate correlation, please use Cohen (1988)

| Coeffficient Value | Strength of Association |
| :----------- | :----------- |
| 0.1 < _r_ < .3 | small correlation |
| 0.3 < _r_ < .5 | medium/moderate correlation |
| _r_ > .5 | Large/strong correlation |

----

**B. Research Questions**^[If a `parametric` test is selected, do not forget to run the normality test.]

***Research Question #1***

It has been published in the press that players from `Iowa State` work harder during practice compared to players from any other college teams in the country. 
...
> You will need to Filter cases to perform this analysis so that only values `69` and `70` is selected for `college`. If a parametric test is selected/checked.
...
> You will need to Filter cases to perform this analysis so that only the value `3` is selected/checked for `position`. 

***Research Question #3***

Defensive players are known to be stronger than those playing on other positions. But how about if we compare defense players among themselves from different positions?

Are Defensive `Tackles (4)`, `Linebackers (10)`, and `Cornerbacks (2)` different when it comes to `bench press (pre)` scores?

> You will need to Filter cases to perform this analysis so that only the valuea `4`, `10` and `2` are selected/checked for `position`. 
...
Since the Wonderlic^[https://en.wikipedia.org/wiki/Wonderlic_test] test assesses cognitive ability under pressure, an interesting question is whether QBs are above average when it comes to cognitive ability. 
...
> For this analysis, use the variables `40-yard Shuttle`, and filter cases so that only position `18` is selected/checked. Also, note that this study involves only players who were tested in `2020`.
...
> For this analysis, use the variables `3-cone Drill`, and filter cases so that only position `17` is selected/checked. Also, note that this study involves only players who were tested in `2020`.
...
> For this analysis, use the variables `3-cone Drill`, and filter cases so that only position `18` is selected/checked. Also, note that this study involves only players who were tested in `2020`.
